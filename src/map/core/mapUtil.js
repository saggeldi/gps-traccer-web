...
export const findFonts = (map) => {
  try {
    ...
    // Handle specific font servers
    if (glyphs.includes('tiles.openfreemap.org')) {
      return ['Noto Sans Regular'];
    }
    if (glyphs.includes('maps.balary.net') || glyphs.includes('demotiles.maplibre.org')) {
      return ['Noto Sans Regular'];
    }
    if (glyphs.includes('cdn.traccar.com')) {
      return ['Open Sans Regular'];
    }
    
    // Default fallback fonts that work with most font servers
    return ['Noto Sans Regular', 'Open Sans Regular', 'Arial Unicode MS Regular'];
  } catch (error) {
    console.warn('Error getting fonts from map style:', error);
    return ['Noto Sans Regular', 'Open Sans Regular', 'Arial Unicode MS Regular'];
  }
};
